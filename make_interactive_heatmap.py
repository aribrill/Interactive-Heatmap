...
def make_heatmap(data_dir, runID, eventID):
    filename = os.path.join(data_dir, "run{}.h5".format(runID)) # calls file from folder run_files with name run######.h5 where ###### is the run ID
    print("Reading file: {}".format(filename))
    one_event = False if eventID is None else True
    wf = sct_toolkit.waveform(filename) # calls the script waveform from sct_toolkit folder. waveform takes in raw or calibrated waveform data and applies pedistal subtraction.
    camera_facing = True # If true, view is from the front of the camera. If false it is from the back.
    ...
    indices = np.arange(total_cells).reshape(-1,int(np.sqrt(total_cells))) # creates square matrix of channel indices for all channels in all modules
    ...
    for mod in wf.get_module_list(): #wf.get_module_list() returns a list of the modules which were included during data-taking
        ...
        for asic in wf.get_asic_list(): # get_asic_list() returns a list of the ASICs which were included during data-taking
            for ch in wf.get_channel_list(): # get_channel_list() returns a list of the channels which were included during data-taking
    	    	charge = np.array(wf.get_branch('Module{}/Asic{}/Channel{}/charge'.format(mod,asic,ch))) # get list of charges in ADC*ns with 16ns integration window
                waveform = np.array(wf.get_branch('Module{}/Asic{}/Channel{}/waveform'.format(mod,asic,ch))) # get list of calibrated (ped-subtracted) waveform. #FIXME change waveform to cal_waveform if using pedistal subtraction
    	    ...
    	    # Modification if only selecting one event at a time! #FIXME
                if one_event==True:
    		    pixels[grid_ind] = charge[eventID] # add the charge of just the specified event
    ...
    if one_event:
        output_filename = str(runID) + '_' + str(eventID) + '.html'
    else:
        output_filename = str(runID) + '.html'
    output_file(output_filename, title="Run ID: "+str(runID)) # creates file which will be filled shortly
...
def main():
    ...
    eventID = int(argList[2]) if len(argList) > 2 else None
    #argList[2] means the third thing that was typed into the terminal. Here it is optional to specify an individual event
    
    make_heatmap("run_files", runID, eventID)
